...
CHTTPParser::CHTTPParser( DataValues *dv):
m_dv(dv)
{    
}
...
void CHTTPParser::S_page_all(QTcpSocket *socket)
{
  S_header(socket);
  //S_page_progs( client  , "");
  //S_page_rooms( client  , "");
...
void CHTTPParser::S_header(QTcpSocket *socket)
{
  std::string page;
  page = "HTTP/1.1 200 OK"
         "Content-Type: text/html";
  socket->write(page.c_str());
  socket->write(""); //  do not forget this one
...
//  socket->write(page);
//  socket->write( DT.m_log.getHTML1() );
//  socket->write( DT.m_log.getHTML2() );
